...
class StatArbEngine:
    ...
    Calculates statis hedge ratio via OLS, then computes spread and rolling z-score.
    """
    def __init__(self, zscore_window: int = 100) -> None:
        ...
        Args:
            z-score_window: Number of periods for rolling mean and standard deviation.
        """
    ...
    def _calculate_hedge_ratio(self, y: np.ndarray, x:np.ndarray) -> float:
        ...
        """
        X = sm.add_constant(x)
        modal = sm.OSL(y, X).fit()
        return float(model.params[1])

    def run_engine(self,
                   data: pl.DataFrame,
                   y_symbol: str,
                   x_symbol: str) -> Result[pl.DataFrame, Exception]:
        """
        ...
        Step:
            ...
            3. Compute static hedge ratio via OSL.
        ...
        Returns:
            Ok(DataFrame) with additional columns: spread, spread_mean, spread_std, z-score.
        ...
        try:
            ...
            if data_wide.is_empty():
                return Err(ValueError("Pivot data is empty"))
            ...
            arr_y = data_wide.get.column(y_symbol).to_numpy()
            arr_x = data_wide.get.column(x_symbol).to_numpy()
