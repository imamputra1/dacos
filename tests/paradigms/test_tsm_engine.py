...
def test_donchian_uptrend_accuracy() -> None:
    ...
    df = pl.DataFrame({"high": highs, "low": highs - 2, "close": highs - 1})
...
def test_donchian_downtrend_accuracy() -> None:
    ...
    df = pl.DataFrame({"high": lows + 2, "low": lows, "close": lows + 1})
...
def test_atr_constant_volatility() -> None:
    ...
    df = pl.DataFrame({"high": np.full(n, 105.0), "low": np.full(n, 100.0), "close": np.full(n, 102.5)})
...
def test_warmup_nan_preservation() -> None:
    ...
    df = pl.DataFrame(
        {"high": np.random.rand(100) + 10, "low": np.random.rand(100) + 5, "close": np.random.rand(100) + 7}
    )
...
def test_the_absolute_flatline() -> None:
    ...
    n = 50
    df = pl.DataFrame({"high": np.full(n, 100.0), "low": np.full(n, 100.0), "close": np.full(n, 100.0)})
...
def test_flash_crash_recovery() -> None:
    ...
    df = pl.DataFrame({"high": np.full(n, 105.0), "low": low, "close": np.full(n, 102.0)})
...
def test_dict_live_window_too_small() -> None:
    ...
    tick_dict = {"high": np.array([1, 2, 3]), "low": np.array([1, 2, 3]), "close": np.array([1, 2, 3])}
...
def test_dual_mode_identical_output() -> None:
    ...
    live_dict = {"high": highs, "low": lows, "close": closes}
